...
export function ParallaxGrid({ posts, className, base = 'blog' }: ParallaxGridProps) {
  ...
  useGSAP(() => {
    ...
    // Mobile: simple reveal, no parallax/mask
    mm.add("(max-width: 767px)", () => {
      const items = containerEl.querySelectorAll(".parallax-item");
      items.forEach((item, index) => {
        ...
        if (!imageContainer || !content || !image) return;

        // Ensure visible without clip-path masking
        gsap.set(imageContainer, { opacity: 0, clearProps: "clipPath" });
        gsap.set(image, { y: 18, scale: 1.02, opacity: 0 });
        ...
        gsap.to(imageContainer, {
          ...
          ease: "power2.out",
          scrollTrigger: {
            id: `parallax-mobile-container-${index}`,
            trigger: item,
            start: "top 85%",
            once: true,
          },
        ...
        gsap.to(image, {
          ...
          ease: "power3.out",
          scrollTrigger: {
            id: `parallax-mobile-image-${index}`,
            trigger: item,
            start: "top 85%",
            once: true,
          },
        ...
        gsap.to(content, {
          ...
          ease: "power2.out",
          scrollTrigger: {
            id: `parallax-mobile-content-${index}`,
            trigger: item,
            start: "top 85%",
            once: true,
          },
      ...
      return () => {
        ScrollTrigger.getAll().forEach((t) => {
          if (typeof t.vars.id === "string" && t.vars.id.startsWith("parallax-mobile-")) {
            t.kill();
          }
        });
      };
    ...
    // Desktop: original parallax + mask
    mm.add("(min-width: 768px)", () => {
      const parallaxItems = containerEl.querySelectorAll(".parallax-item");
      ...
      const initializeAnimations = () => {
        parallaxItems.forEach((item, index) => {
          ...
          // Hover bubble and ripple remain for desktop
      ...
      // Ensure images loaded for desktop sequence
      const checkImagesLoaded = () => {
        const images = containerEl.querySelectorAll(".parallax-image-wrapper img");
        ...
        images.forEach((img) => {
          ...
          if (el.complete && el.naturalWidth > 0) {
            ...
            el.addEventListener("load", () => {
              loadedCount++;
              if (loadedCount === totalImages) initializeAnimations();
            }, { once: true });
